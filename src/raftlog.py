...
E        +  where False = <function append_entries at 0x109f5c400>([LogEntry(term=1, item='1'), LogEntry(term=1, item='1'), LogEntry(term=1, item='1'), LogEntry(term=4, item='4'), LogEntry(term=4, item='4'), LogEntry(term=5, item='5'), ...], 9, 6, [LogEntry(term=6, item='6')])
E        +    where <function append_entries at 0x109f5c400> = raftlog.append_entries
...
def append_entries(
    ...
    if prev_log_index > 0 and (prev_log_index > len(log) or log[prev_log_index - 1].term != leader_term - 1):
        return False
